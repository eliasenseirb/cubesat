...
Nb_preambule_up = 8:24:32; % Preambule    % Faire varier
...
for boucle=1:var_boucle
    ...
    for NpBoucle=1:length(Nb_preambule_up)
        ...
        for k=1:length(gammap)
            s = [s exp(1j*fc(time_upsampled,gammap(k),B,Ts,Dp(k),M))]; % génération des chirps
        end
        %[s, ~, ~]=LoraTxI2SC(sb,SF,B,Fse,M,Ts,Te);
        s=[preambule s];
        ...
        for i=1:length(eb_n0_dB)
            ...
            %figure,plot(abs(fft(x2(1:M).*conj(chirp_up))))
            %figure,plot(abs(fft(x(1:M*Fse).*conj(chirp_up_upsampled))))
            %%
            [DR_esti,test] = doppler_rate_esti(x2,M,Nb_preambule_up(NpBoucle),chirp_up,Ts,1); %estimation doppler rate

            erreur_quadratique(NpBoucle,i) =(erreur_quadratique(NpBoucle,i)+ (Dr_simul-DR_esti)^2)/2;
            erreur_standard(NpBoucle,i) = (erreur_standard(NpBoucle,i)+abs(Dr_simul-DR_esti))/2;
...
figure(1),semilogy(eb_n0_dB,erreur_quadratique),grid on,title('Squared Error'),legend('Np=8','Np=32')%'Np=4','Np=8','Np=12')%,'Np=16','Np=20','Np=24','Np=28','Np=32')
xlabel('$eb\_n0\_dB$','Interpreter','latex','fontsize',14),ylabel('$(\hat{D}_p-D_p)^2$','Interpreter','latex','fontsize',14)
figure(2),semilogy(eb_n0_dB,erreur_standard),grid on,title('Standard Error'),legend('Np=8','Np=32')%,'Np=16','Np=20','Np=24','Np=28','Np=32')
...
% g=gausswin(20);
% g=g/sum(g);
% test=conv2(erreur_standard(1,:),g.');
% figure,semilogy(eb_n0_dB,flip(test(1:length(eb_n0_dB))))