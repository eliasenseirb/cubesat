...
function [H] = H_w(Np,Nsw,w,x,M,Fse,time_upsampled,B,Ts) % Np : nombres de chirps up, Nsw : nombres de chirps de synchro, w : indice ou calculer la fonction
% x :signal
    nb_colonnes = Np+Nsw-1;
    if w<0
        %x=[exp(1j*2*pi.*time_upsampled.*fc(time_upsampled,0,B,Ts)),x];
        x=[zeros(1,M*Fse),x]; % on rajoute des 0 devant si le préambule se situe dans le premier paquet de M chirps
        w=1+w;
    end
    vect = zeros(1,nb_colonnes);
    for i =1:nb_colonnes
        %figure,plot(abs(fft(x((i+w-1)*M*Fse+1:(i+w)*M*Fse))))
        vect(i) = max(abs(fft(x((i+w-1)*M*Fse+1:(i+w)*M*Fse)))); % calcul des max des fft des paquets de M chirps
    end
